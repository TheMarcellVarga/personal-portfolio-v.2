...
export default function Page() {
  ...
  const setActiveDarkMode = useCallback(() => {
    // TODO: Remove darkModeKey when we replace MUI with JOY UI
    if (
      (mode === "dark" && isDarkModeKey) ||
      (mode === "light" && !isDarkModeKey)
    ) {
      dispatch(toggleDarkModeKey());
    }
    setMode(mode === "dark" ? "light" : "dark"); // We need to use `setMode`, otherwise theme.palette.mode won't work
  }, [dispatch, setMode, mode, isDarkModeKey]);
  ...
  return (
    <div className="mx-4 mb-4" data-theme={theme}>
      <Header />
      ...
      <section className="flex flex-col items-center justify-between h-screen m-4 gap-1 w-full">
        <div className="flex w-full h-screen text-5xl font-medium items-center justify-start">
          <h2 className="text-2xl font-light text-justify m-6 leading-relaxed">
              ...
              <div key={index} className="flex">
                ...
                <div className="w-1/2">
                  <div className="flex flex-col gap-2">
                    <h3 className="text-lg">{item.jobTitle}</h3>
