...
function useSmoothScroll() {
  const requestAnimationFrameId = useRef<number>(null);

  const smoothScroll = useCallback((targetY: number) => {
    const startTime = performance.now();

    const frameFunc = (currentTime: number) => {
      const elapsed = currentTime - startTime;

      const t = Math.min(elapsed / 1000, 1); // Ensure t is between 0 and 1

      const easing = t < 0.5 ? 2 * t * t : -2 * t * t + 4 * t - 1;

      const newY = easing * (targetY - window.pageYOffset) + window.pageYOffset;
      ...
      if (t < 1) {
        requestAnimationFrameId.current = requestAnimationFrame(frameFunc);
      } else {
...
export default function Page() {
  ...
  const checkScrollTop = useCallback(() => {
    if (!isAnimating && window.pageYOffset > 200 && window.pageYOffset < 800) {
      setIsAnimating(true);
      smoothScroll(400); // Scroll to position 400 (adjust as needed)
      setTimeout(() => setIsAnimating(false), 1000); // Reset after animation completes
    }
  }, [isAnimating, smoothScroll]);
  ...
  return (
    ...
    >
      ...
      <main className="flex flex-col items-center justify-between h-screen m-4 gap-1">
        <div className="flex-grow pb-12 w-full flex flex-row items-center justify-strech gap-2 fade-top-bottom">
          <div className="absolute w-fit z-20 pl-24">
            ...
            <div className="flex w-full text-4xl font-extralight items-center justify-start my-4 text-slate-900 dark:text-slate-50 ">
      ...
      <section className="flex flex-col items-center justify-between h-screen gap-1 w-full ">
        <div className="flex w-4y/5 h-screen text-5xl font-medium items-center justify-start">
