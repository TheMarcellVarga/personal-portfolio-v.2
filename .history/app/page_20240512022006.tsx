...
function MyScene({ isDarkMode }: { isDarkMode: boolean }) {
  const { camera, gl } = useThree();
  const size = useWindowSize(); // Assuming you have this hook to get window size
  ...
  useEffect(() => {
    ...
    controls.minDistance = 10; // Set the minimum distance for zooming
    controls.maxDistance = 100; // Set the maximum distance for zooming
    return () => controls.dispose(); // Clean up on unmount
  }, [camera, gl]);
