  ...
  const [prefersDarkMode, setPrefersDarkMode] = useState(false);
    ...
    useEffect(() => {
      ...
      if (typeof window !== "undefined") {
        ...
        const listener = (e: MediaQueryListEvent) => setPrefersDarkMode(e.matches);
        mediaQuery.addListener(listener);
        return () => mediaQuery.removeListener(listener);
      }
    }, []);

    // Use prefersDarkMode state to apply dark mode styles
    // For example, you can conditionally apply a 'dark' class to the body element
    useEffect(() => {
      if (prefersDarkMode) {
