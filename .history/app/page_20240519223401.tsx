...
export default function Page() {
  ...
  useEffect(() => {
    if (globeRef.current) {
      ...
      // Start auto-rotation
      const rotateGlobe = () => {
        // Update the longitude to create rotation effect
        // Note: You might need to adjust the increment value for smoother rotation
        const pov = globeRef.current.pointOfView();
        globeRef.current.pointOfView({
         ...pov,
          lng: pov.lng + 0.01, // Increment longitude
        });
  
        // Request animation frame to smoothly rotate the globe
    ...
    // Cleanup function to stop rotation when the component unmounts
    return () => cancelAnimationFrame(rotateGlobe);
  }, []);
