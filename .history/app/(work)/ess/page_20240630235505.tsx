                      ...
                      <div className="text-sm text-black dark:text-white ">
                        {Array.isArray(item.description)? (
                          ...
                          <div key={descIndex}>
                            ...
                            <div className="flex flex-wrap"> {/* Ensure flex-wrap is used to handle multiple rows if needed */}
                              {Array.isArray(desc.picture)? (
                                desc.picture.map((pictureUrl, pictureIndex) => {
                                  const totalPictures = desc.picture.length;
                                  const widthFraction = `w-${Math.floor(100 / totalPictures)}%`; // Calculate the width fraction based on total pictures
                                  return (
                                    <div key={pictureIndex} className={`${widthFraction}`} style={{ flexBasis: `${widthFraction}` }}> {/* Apply the width fraction */}
                                      <Image
                                        src={pictureUrl}
                                        alt="Description"
                                        layout="responsive"
