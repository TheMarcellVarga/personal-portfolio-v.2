                              ...
                              {item.description.map((desc, descIndex) => (
                                <div key={descIndex}>
                                    {Array.isArray(desc.picture)? (
                                      <div className="flex flex-col gap-4"> {/* Default to flex-col, adjust dynamically below */}
                                        {Array.isArray(desc.picture) && desc.picture.map((pictureItem, pictureIndex) => {
                                          // Determine layout based on the first item's bySide property if it's an object
                                          const isBeside = Array.isArray(pictureItem) && typeof pictureItem[0] === 'object' && pictureItem[0].hasOwnProperty('bySide') && pictureItem[0].bySide === "true";
                                          const layoutClass = isBeside? "flex-row" : "flex-col"; // Use flex-row if bySide is true, otherwise flex-col

                                          return (
                                            <div key={descIndex} className={`flex ${layoutClass} gap-4`}>
                                              {desc.picture.map((pictureObj, pictureIndex) => {
                                                if (typeof pictureObj === 'string') {
                                                  // Directly use the string as the src for an img tag
                                                  return (
                                                    <div key={pictureIndex} className="w-full object-cover">
                                                      <img src={pictureObj} alt="" className="w-full h-auto" />
                                                    </div>
                                                  );
                                                } else {
                                                  // Safely access src assuming pictureObj is an object with bySide and src properties
                                                  return (
                                                    <div key={pictureIndex} className="w-full object-cover">
                                                      <img src={pictureObj.src} alt="" className={`w-${isBeside? "1/2" : "full"} h-auto`} />
                                                    </div>
                                                  );
                                                }
                                              }))
