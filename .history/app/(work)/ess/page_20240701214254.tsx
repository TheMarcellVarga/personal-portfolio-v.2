...
{history.map((item, index) => (
  <div key={index} className="flex">
    <div className="flex flex-col gap-2">
      ...
      {/* Assuming desc is an object within the description array of each item */}
      {item.description.map((desc, descIndex) => (
        <div key={descIndex} className="mt-2">
          <p className="text-lg">{desc.title}</p>
          <div className="py-2">
          ...
          {/* Render subTitle smaller than the normal title */}
          ...
          {desc.subPicture.length > 0 && (
            <div className="mt-2 flex flex-row space-x-4">
              {desc.subPicture.map((pictureUrl: string, pictureIndex: number) => (
                <div key={pictureIndex} className="w-full max-w-xs">
                  <Image
                    src={pictureUrl}
