...
const ESS = () => {
  ...
  <div className="py-2">
    {Array.isArray(desc.text) && Array.isArray(desc.picture) ? (
      <>
        {Array.from({ length: Math.max(Math.max(desc.text.length, desc.picture.length, desc.subText?.length ?? 0, desc.subPicture?.length ?? 0) }).map((_, index) => (
          <React.Fragment key={index}>
            {/* Render text if available */}
            {index < desc.text.length && (
              <p>{desc.text[index]}</p>
            )}

            {/* Render picture if available */}
            {index < desc.picture.length && (
              <div className="mt-2 flex flex-wrap justify-center items-start w-full">
                {Array.isArray(desc.picture[index]) ? (
                  desc.picture[index].map((pictureUrl, pictureIndex) => (
                    <Image
            ...
            {desc.subText && index < desc.subText.length && (
              <p>{desc.subText[index]}</p>
            )}

            {/* Render subPicture if available */}
            {desc.subPicture && index < desc.subPicture.length && (
              <div className="mt-2 flex flex-wrap justify-center items-start w-full">
                {Array.isArray(desc.subPicture[index]) ? (
                  desc.subPicture[index].map((pictureUrl, pictureIndex) => (
                    <Image
                      key={`${index}-${pictureIndex}`}
                      src={pictureUrl}
                      alt={`Sub-Picture ${index + 1}`}
                      layout="fixed"
                      width={150}
                      height={100}
                      objectFit="cover"
      ...
      // Fallback rendering if desc.text or desc.picture is not an array
      <p>{desc.text || ""}</p>
    )}
  </div>
</div>                        ))}{" "}
