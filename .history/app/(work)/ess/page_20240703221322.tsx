                      ...
                      <div className="text-sm text-black dark:text-white ">
                        {item.description.map((desc, descIndex) => (
                          <div key={descIndex}>
                            ...
                            <div className="py-2">
                              const combinedContent = desc.text.map((text, index) => ({
                                type: 'text',
                                content: text,
                              }));

                              desc.picture.forEach((picture, index) => {
                                if (typeof picture === 'string') {
                                  combinedContent.splice(index * 2 + 1, 0, {
                                    type: 'singlePicture',
                                    content: picture,
                                  });
                                } else if (Array.isArray(picture)) {
                                  picture.forEach((picUrl, picIndex) => {
                                    combinedContent.splice(index * 2 + 1 + picIndex, 0, {
                                      type: 'groupPicture',
                                      content: picUrl,
                                    });
                                  });
                              ...
                              return (
                                <div className="mt-2 flex flex-wrap justify-center items-start w-full">
                                  {combinedContent.map((item, itemIndex) => {
                                    switch (item.type) {
                                      case 'text':
                                        return (
                                          <div key={itemIndex} className="w-full text-base mb-4">
                                            {item.content}
                                          </div>
                                        );
                                      case 'singlePicture':
