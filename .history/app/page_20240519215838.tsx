...
export default function Page() {
  ...
  useEffect(() => {
    const rotateGlobe = () => {
      if (globeRef.current) {
        // Use the pointOfView method to rotate the globe
        // The pointOfView method takes a GeoCoords object and an optional transition duration (in milliseconds)
        // For example, to rotate the globe by 10 degrees every second:
        globeRef.current.pointOfView({ lat: 0, lng: 10 }, 1000);
      }
    };

    // Call rotateGlobe every second
    const intervalId = setInterval(rotateGlobe, 1000);
