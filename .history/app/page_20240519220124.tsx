...
export default function Page() {
  ...
  const [pointsData, setPointsData] = useState([
    { lat: 55.6761, lng: 12.5683, size: 1, color: 'red', name: 'Copenhagen, Denmark' },
    // Add more points as needed
  ]);

  const globeRef = useRef<GlobeMethods | null>(null);

  const componentMounted1 = useRef(() => {
    if (componentMounted1.current) {
      // Now it's safe to access globeRef.current
      const rotatePoints = () => {
        // Calculate the new longitude for each point
        const newLng = pointsData[0].lng + 0.01; // Adjust the value as needed
        setPointsData(pointsData.map(point => ({...point, lng: newLng }));
      };

      // Call rotatePoints every second
      const intervalId = setInterval(rotatePoints, 1000);
  ...
  const componentMounted2 = useRef(() => {
    if (componentMounted2.current) {
      // Now it's safe to access globeRef.current
      const rotatePoints = () => {
        // Calculate the new longitude for each point
        const newLng = pointsData[0].lng + 0.01; // Adjust the value as needed
    ...
    <div className="px-4 pb-4 bg-custom-light dark:bg-custom-dark transition-colors duration-200 ease-in-out">
            ...
            <div className="w-full flex flex-col gap-12">
              {projects.map((project) => (
                <div key={project.title} className="w-full flex">
                  ...
                  <Link
                    href={project.link}
                    className="w-full flex flex-row bg-gray-50 p-8 rounded-lg hover:bg-gray-100 hover:border-gray-200 border-2 border-transparent transition-colors duration-200 ease-in-out"
                  >
                        ...
                        <div className="w-full h-full flex justify-center items-center">
                          <div className="w-[300px] h-[200px] relative">
                            <Image
                              ...
                              objectFit="contain" // Ensure the image fits within the container without being cropped or stretched
                            />
                          </div>
                        </div>
                      </div>
      ...
      <section>
        <div className="flex w-full items-center justify-center text-5xl mt-60">
          <h3 className="text-2xl font-light text-justify m-6 leading-relaxed text-black dark:text-white ">
            Current Location:&nbsp;
