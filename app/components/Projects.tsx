  ...
  return (
    ...
    >
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-8">
        ...
        <div className="relative mt-4 sm:mt-0" data-filter-container>
          <button 
            onClick={() => setIsFilterOpen(!isFilterOpen)}
            className="flex items-center gap-2 px-4 py-2 text-sm text-text-primary bg-background-secondary rounded-md border border-border-color hover:bg-background-secondary/80 transition-colors"
          ...
          {isFilterOpen && (
            <div className="absolute z-10 mt-1 w-full rounded-md shadow-lg bg-background-primary border border-border-color max-h-60 overflow-auto">
              <ul className="py-1" role="listbox">
                {allSkills.map(skill => (
                  <li 
                    key={skill}
                    role="option"
                    aria-selected={selectedFilter === skill}
                    onClick={() => {
                      setSelectedFilter(skill);
                      setIsFilterOpen(false);
                    }}
                    className={`
                      px-4 py-2 text-sm cursor-pointer hover:bg-background-secondary
                      ${selectedFilter === skill ? 'bg-background-secondary text-text-primary' : 'text-text-secondary'}
                    `}
                  >
                    {skill}
                  </li>
        ...
        <div className="w-full flex flex-col gap-12">
          {filteredProjects.length > 0 ? (
            ...
            <div className="text-center py-10">
              <p className="text-text-secondary text-lg">No projects found with the selected skill.</p>
              <button 
