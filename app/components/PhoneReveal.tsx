...
export const PhoneReveal = forwardRef<PhoneRevealHandle, PhoneRevealProps>(function PhoneReveal(
  ...
  return isVisible && phone ? (
    <a
      href={`tel:${phone}`}
      className={[
        "flex items-start gap-2 text-[10px] leading-[1.45] text-white/66 transition-colors hover:text-white",
        className ?? "",
      ]
        .filter(Boolean)
        .join(" ")}
    ...
    <button
      ...
      onClick={revealPhone}
      className={[
        "flex items-start gap-2 text-left text-[10px] leading-[1.45] text-white/66 transition-colors hover:text-white",
        className ?? "",
      ]
        .filter(Boolean)
        .join(" ")}
