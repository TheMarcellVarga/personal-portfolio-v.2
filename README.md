...
The site exposes email and professional social links, with an optional click-to-reveal phone interaction. The reveal keeps the number out of initial page markup but is not scrape-proof; private environment values and credentials must never be committed. PostHog, Vercel Analytics, and Speed Insights are loaded without an interrupting consent popover and collect pageviews, autocapture, session recording, performance, console logs, and error measurements when their production configuration is present. Inputs are masked in session recordings. The Privacy link in the footer explains the analytics behavior. The short-lived homepage intro cookie only remembers whether the visual intro has completed and is not used for measurement.
