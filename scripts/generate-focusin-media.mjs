...
const opaquePanelEvidenceRoot = path.join(focusinRoot, "docs/evidence/opaque-panels");
const outputRoot = path.join(repoRoot, "public/images/focusin");

const sources = {
  focusing: path.join(opaquePanelEvidenceRoot, "light-normal-focusing.png"),
  breakDue: path.join(opaquePanelEvidenceRoot, "light-normal-break-due.png"),
  breaking: path.join(opaquePanelEvidenceRoot, "dark-normal-breaking.png"),
...
async function roundedScreenshot(input, width, height, radius = 30) {
  const image = await resized(input, width, height);
...
async function writeHero() {
  const width = 2400;
  const height = 1350;
  const focusing = await roundedScreenshot(sources.focusing, 790, 960);
  const breaking = await roundedScreenshot(sources.breaking, 760, 1008);
...
async function writeBreakSequence() {
  const width = 2000;
  const height = 1300;
  const breakDue = await roundedScreenshot(sources.breakDue, 760, 922);
  const breaking = await roundedScreenshot(sources.breaking, 700, 928);
  const background = svg(
    width,
    height,
